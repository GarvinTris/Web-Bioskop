...
-- 1. Tabel kategori
CREATE TABLE web_bioskop_kategori (
    ID_Kategori VARCHAR(20) PRIMARY KEY,
    Nama_Kategori VARCHAR(50) NOT NULL
);

-- 2. Tabel film
CREATE TABLE web_bioskop_film (
    ...
    Rating DECIMAL(2,1),
    FOREIGN KEY (ID_Kategori) REFERENCES web_bioskop_kategori(ID_Kategori) ON DELETE SET NULL
);

-- 3. Tabel studio
CREATE TABLE web_bioskop_studio (
...
CREATE TABLE web_bioskop_jadwal (
    ...
    FOREIGN KEY (No_Studio) REFERENCES web_bioskop_studio(No_Studio) ON DELETE CASCADE,
    FOREIGN KEY (ID_Film) REFERENCES web_bioskop_film(ID_Film) ON DELETE CASCADE
);

-- 6. Tabel penonton
CREATE TABLE web_bioskop_penonton (
...
-- 7. Tabel tiket
CREATE TABLE web_bioskop_tiket (
    ...
    FOREIGN KEY (ID_Kursi) REFERENCES web_bioskop_kursi(ID_Kursi) ON DELETE CASCADE,
    FOREIGN KEY (ID_Jadwal) REFERENCES web_bioskop_jadwal(ID_Jadwal) ON DELETE CASCADE,
    UNIQUE KEY unique_tiket (ID_Jadwal, ID_Kursi)
);

-- 8. Tabel transaksi
CREATE TABLE web_bioskop_transaksi (
    ...
    FOREIGN KEY (ID_Jadwal) REFERENCES web_bioskop_jadwal(ID_Jadwal) ON DELETE CASCADE,
    FOREIGN KEY (ID_Penonton) REFERENCES web_bioskop_penonton(ID_Penonton) ON DELETE CASCADE
);

INSERT INTO web_bioskop_film (Judul_Film, Durasi, ID_Kategori, image, Director, Deskripsi, Rating_Usia, Trailer_URL, Rating) VALUES
...
INSERT INTO web_bioskop_kategori (ID_Kategori, Nama_Kategori) VALUES
('AKSI', 'Action'),
('ANIM', 'Animation'),
('DOKUM', 'Dokumenter'),
('DRAMA', 'Drama'),
...
INSERT INTO web_bioskop_kursi (ID_Kursi, No_Studio, Baris, Nomor_Kursi) VALUES

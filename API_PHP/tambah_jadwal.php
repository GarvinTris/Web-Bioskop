...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Validasi
    if (!$id_film || !$tanggal || !$jam_mulai || !$no_studio || !$harga) {
    ...
    if ($id_jadwal) {
        ...
        $stmt->bind_param("ssssis", $id_film, $tanggal, $jam_mulai, $no_studio, $harga, $id_jadwal);
        ...
        $query = "INSERT INTO jadwal (ID_Jadwal, ID_Film, Tanggal, Jam_Mulai, No_Studio, Harga) 
                  VALUES (?, ?, ?, ?, ?, ?)";
        
        $stmt = $conn->prepare($query);
        $stmt->bind_param("sssssi", $new_id, $id_film, $tanggal, $jam_mulai, $no_studio, $harga);
